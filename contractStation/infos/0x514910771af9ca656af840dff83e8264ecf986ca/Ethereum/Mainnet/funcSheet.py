...
def name():
	...
	return tx
def approve(ls):
	address___spender,uint256___value=ls
	tx = cont.functions.approve(str(address___spender),int(uint256___value)).buildTransaction({'gasPrice': avgGasPriceEst(),'gas':web3.eth.estimateGas({'to': w3.toChecksumAddress(account_1.address), 'from': w3.toChecksumAddress(add)}),'from': account_1.address,'nonce': nonce,'chainId': int(chainId)})
	print(tx)
	return tx
def totalSupply():
	tx = cont.functions.totalSupply().call()
	print("totalSupply ="+str(tx))
	return tx
def transferFrom(ls):
	address___from,address___to,uint256___value=ls
	tx = cont.functions.transferFrom(str(address___from),str(address___to),int(uint256___value)).buildTransaction({'gasPrice': avgGasPriceEst(),'gas':web3.eth.estimateGas({'to': w3.toChecksumAddress(account_1.address), 'from': w3.toChecksumAddress(add)}),'from': account_1.address,'nonce': nonce,'chainId': int(chainId)})
	print(tx)
	return tx
def decimals():
	tx = cont.functions.decimals().call()
	print("decimals ="+str(tx))
	return tx
def transferAndCall(ls):
	address___to,uint256___value,bytes___data=ls
	tx = cont.functions.transferAndCall(str(address___to),int(uint256___value),kek(bytes___data)).buildTransaction({'gasPrice': avgGasPriceEst(),'gas':web3.eth.estimateGas({'to': w3.toChecksumAddress(account_1.address), 'from': w3.toChecksumAddress(add)}),'from': account_1.address,'nonce': nonce,'chainId': int(chainId)})
	print(tx)
	return tx
def decreaseApproval(ls):
	address___spender,uint256___subtractedValue=ls
	tx = cont.functions.decreaseApproval(str(address___spender),int(uint256___subtractedValue)).buildTransaction({'gasPrice': avgGasPriceEst(),'gas':web3.eth.estimateGas({'to': w3.toChecksumAddress(account_1.address), 'from': w3.toChecksumAddress(add)}),'from': account_1.address,'nonce': nonce,'chainId': int(chainId)})
	print(tx)
	return tx
def balanceOf(ls):
	address___owner=ls[0]
	tx = cont.functions.balanceOf(str(address___owner)).buildTransaction({'gasPrice': avgGasPriceEst(),'gas':web3.eth.estimateGas({'to': w3.toChecksumAddress(account_1.address), 'from': w3.toChecksumAddress(add)}),'from': account_1.address,'nonce': nonce,'chainId': int(chainId)})
	print(tx)
	return tx
def symbol():
	...
	return tx
def transfer(ls):
	address___to,uint256___value=ls
	tx = cont.functions.transfer(str(address___to),int(uint256___value)).buildTransaction({'gasPrice': avgGasPriceEst(),'gas':web3.eth.estimateGas({'to': w3.toChecksumAddress(account_1.address), 'from': w3.toChecksumAddress(add)}),'from': account_1.address,'nonce': nonce,'chainId': int(chainId)})
	print(tx)
	return tx
def increaseApproval(ls):
	address___spender,uint256___addedValue=ls
	tx = cont.functions.increaseApproval(str(address___spender),int(uint256___addedValue)).buildTransaction({'gasPrice': avgGasPriceEst(),'gas':web3.eth.estimateGas({'to': w3.toChecksumAddress(account_1.address), 'from': w3.toChecksumAddress(add)}),'from': account_1.address,'nonce': nonce,'chainId': int(chainId)})
	print(tx)
	return tx
def allowance(ls):
	address___owner,address___spender=ls
	tx = cont.functions.allowance(str(address___owner),str(address___spender)).buildTransaction({'gasPrice': avgGasPriceEst(),'gas':web3.eth.estimateGas({'to': w3.toChecksumAddress(account_1.address), 'from': w3.toChecksumAddress(add)}),'from': account_1.address,'nonce': nonce,'chainId': int(chainId)})
	print(tx)
	return tx

def getDefVaris(na):
	js = {'view_all': '', 'name': '', 'approve': 'address___spender,uint256___value', 'totalSupply': '', 'transferFrom': 'address___from,address___to,uint256___value', 'decimals': '', 'transferAndCall': 'address___to,uint256___value,bytes___data', 'decreaseApproval': 'address___spender,uint256___subtractedValue', 'balanceOf': 'address___owner', 'symbol': '', 'transfer': 'address___to,uint256___value', 'increaseApproval': 'address___spender,uint256___addedValue', 'allowance': 'address___owner,address___spender'}
	return js[na].split(",")
def view_all():
...
def send_it(ans):
    try:
        ...
        print(e)
def getFunList():
    return ['view_all()', 'name()', 'approve(address___spender,uint256___value)', 'totalSupply()', 'transferFrom(address___from,address___to,uint256___value)', 'decimals()', 'transferAndCall(address___to,uint256___value,bytes___data)', 'decreaseApproval(address___spender,uint256___subtractedValue)', 'balanceOf(address___owner)', 'symbol()', 'transfer(address___to,uint256___value)', 'increaseApproval(address___spender,uint256___addedValue)', 'allowance(address___owner,address___spender)']
def hubbub():
  print('\n\n')
  view_all()
  function = askList(getFunList())
  ...
  return 
global netName,chainId,rpc,nativeCurrency,explorer,scanner,w3,nonce,varis,web3
...
changeGlob("jsInfo",{'nativeCurrency': "ETH", 'network': "Mainnet", 'RPC': "https://mainnet.infura.io/v3/9aa3d95b3bc440fa88ea12eaa4456161", 'chainId': 0x1, 'blockExplorer': "https://etherscan.io", 'networkName': "ETH"})
changeGlob('chainId',jsInfo['chainId'])
changeGlob('scanner',jsInfo['blockExplorer'])
changeGlob('w3',Web3(Web3.HTTPProvider(jsInfo['RPC'])))
changeGlob('web3',w3)
gasSpeed = 'safeLow'
add = w3.toChecksumAddress("0x514910771af9ca656af840dff83e8264ecf986ca")
