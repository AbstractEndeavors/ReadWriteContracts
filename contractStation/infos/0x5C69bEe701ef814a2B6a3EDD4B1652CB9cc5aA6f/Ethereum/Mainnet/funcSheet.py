...
def allPairs(ls):
	uint256=ls[0]
	tx = cont.functions.allPairs(int(uint256)).buildTransaction({'gasPrice': avgGasPriceEst(),'gas':web3.eth.estimateGas({'to': w3.toChecksumAddress(account_1.address), 'from': w3.toChecksumAddress(add)}),'from': account_1.address,'nonce': nonce,'chainId': int(chainId)})
	print(tx)
	return tx
def allPairsLength():
	tx = cont.functions.allPairsLength().call()
	print("allPairsLength ="+str(tx))
	return tx
def createPair(ls):
	address__tokenA,address__tokenB=ls
	tx = cont.functions.createPair(str(address__tokenA),str(address__tokenB)).buildTransaction({'gasPrice': avgGasPriceEst(),'gas':web3.eth.estimateGas({'to': w3.toChecksumAddress(account_1.address), 'from': w3.toChecksumAddress(add)}),'from': account_1.address,'nonce': nonce,'chainId': int(chainId)})
	print(tx)
	return tx
def feeTo():
	tx = cont.functions.feeTo().call()
	print("feeTo ="+str(tx))
	return tx
def feeToSetter():
	tx = cont.functions.feeToSetter().call()
	print("feeToSetter ="+str(tx))
	return tx
def getPair(ls):
	address,address=ls
	tx = cont.functions.getPair(str(address),str(address)).buildTransaction({'gasPrice': avgGasPriceEst(),'gas':web3.eth.estimateGas({'to': w3.toChecksumAddress(account_1.address), 'from': w3.toChecksumAddress(add)}),'from': account_1.address,'nonce': nonce,'chainId': int(chainId)})
	print(tx)
	return tx
def setFeeTo(ls):
	address___feeTo=ls[0]
	tx = cont.functions.setFeeTo(str(address___feeTo)).buildTransaction({'gasPrice': avgGasPriceEst(),'gas':web3.eth.estimateGas({'to': w3.toChecksumAddress(account_1.address), 'from': w3.toChecksumAddress(add)}),'from': account_1.address,'nonce': nonce,'chainId': int(chainId)})
	print(tx)
	return tx
def setFeeToSetter(ls):
	address___feeToSetter=ls[0]
	tx = cont.functions.setFeeToSetter(str(address___feeToSetter)).buildTransaction({'gasPrice': avgGasPriceEst(),'gas':web3.eth.estimateGas({'to': w3.toChecksumAddress(account_1.address), 'from': w3.toChecksumAddress(add)}),'from': account_1.address,'nonce': nonce,'chainId': int(chainId)})
	print(tx)
	return tx

def getDefVaris(na):
	js = {'view_all': '', 'allPairs': 'uint256', 'allPairsLength': '', 'createPair': 'address__tokenA,address__tokenB', 'feeTo': '', 'feeToSetter': '', 'getPair': 'address,address', 'setFeeTo': 'address___feeTo', 'setFeeToSetter': 'address___feeToSetter'}
	return js[na].split(",")
def view_all():
	print("allPairsLength",":",(cont.functions.allPairsLength().call()))
	print("feeTo",":",(cont.functions.feeTo().call()))
	print("feeToSetter",":",(cont.functions.feeToSetter().call()))
...
def send_it(ans):
    try:
        ...
        print(e)
def getFunList():
    return ['view_all()', 'allPairs(uint256)', 'allPairsLength()', 'createPair(address__tokenA,address__tokenB)', 'feeTo()', 'feeToSetter()', 'getPair(address,address)', 'setFeeTo(address___feeTo)', 'setFeeToSetter(address___feeToSetter)']
def hubbub():
  print('\n\n')
  view_all()
...
changeGlob('web3',w3)
gasSpeed = 'safeLow'
add = w3.toChecksumAddress("0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f")
