...
def createNewNetwork():
    var = ['name','nativeCurrency','RPC','chainId','blockExplorer']
        ...
        if ans != 'back':
            ...
            if custom == False:
                if len(main) >1:
                    ...
                    main = choose[name][ans2[1]]
            if isLs(main):
                main = main[0]
            chainId,nativeCurrency,explorer,w3 = main['chainId'],main['nativeCurrency'],main['blockExplorer'],Web3(Web3.HTTPProvider(main['RPC']))
            return name,chainId,explorer,main['RPC'],stripWeb(explorer),w3
def get_abi():
    return f.sites('https://'+str(scanners)+'/api?module=contract&action=getabi&address='+str(add)+'&apikey='+str(api_key()))
global name,chainId,rpc,explorer,scanner,w3
homeIt()
